module.exports = async (z) => {
  try {
    ...
    allUsers.sort((a, b) => b.invite.length - a.invite.length);
    
    for (let i = 0; i < 10; i++) {    
      var UsersTop = await z.ctx.telegram.getChatMember(allUsers[i].id, allUsers[i].id);
      txt += `${z.setting.simbol} [${i + 1}]. ${UsersTop.user.first_name}, Invite: ${allUsers[i].invite.length}\n`
    }
    
    z.reply(1, { text: 'Top Mengundang teman terbanyak:\n' + txt, mark: true });
